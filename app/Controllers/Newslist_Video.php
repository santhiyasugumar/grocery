...
class Newslist_Video extends Controller {

    use ResponseTrait;
  
    public function index() {
        echo view('newslist_video_view');
    }
    ...
    public function getAllData() {
        ...
        // $builder->like("cs.cover_title", $search[1]);
        ...
        $sql = "SELECT * FROM news WHERE category_id = $category_id AND established_date <= NOW() AND 
         content LIKE '%$search%' order by created_on desc";
        $query = $db->query($sql);
        $count = $query->getResultArray(); 

        $sql = "SELECT * FROM news WHERE category_id = $category_id AND established_date <= NOW() AND 
         content LIKE '%$search%' order by created_on desc limit $limit offset $offset";
        $query = $db->query($sql);
